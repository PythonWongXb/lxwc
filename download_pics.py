        ...
        if imgs:
            ...
            for i in imgs:
                ...
                if file:
                    ...
                    global path_top
                    # path_top = 'C:\\Users\\Administrator\\Desktop\\pics'
                    path_second = '/' + str(num)
                    path_mid = '/' + str(page)
                    path_third = '/' + str(article_id)
                    file_name = file.rsplit('/', 1)[-1]
                    path = path_top + path_second + path_mid + path_third + '/'
                    # print(file, article_id, page, end=' ')
                    # print(path, file_name)
                    if not os.path.exists(path):
                        os.makedirs(path)
                    # print(file.strip(), path+file_name, )
                    # print(path.strip() + file_name.strip())
                    urlretrieve(file.strip(), path.strip() + file_name.strip())  # 保存到本地，并设置路径
...
if __name__ == '__main__':
    ...
    # 自定自己想放的文件夹路径
    path_top = r'C:/Users/Administrator/Desktop/pics'
    # path_top = input('输入你想下载电脑的位置，回车开始下载')
    # DATABASE = "pic"  # 数据库名字，需要自己先建立，编码为utf8，排序general ci
